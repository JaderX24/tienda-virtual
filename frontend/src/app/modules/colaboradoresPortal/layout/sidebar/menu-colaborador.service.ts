...
import { SeccionMenuColab } from './menu.config';

@Injectable({
    providedIn: 'root',
})
export class MenuColaboradorService {
    private readonly http = inject(HttpClient);
    private readonly authService = inject(AuthColaboradorService);
    private readonly apiUrl = `${environment.apiUrl}/colaborador/menu`;

    private readonly menuSecciones = signal<SeccionMenuColab[]>([]);
    ...
    cargarMenu(): void {
        ...
        this.http.get<{ exito: boolean; datos: SeccionMenuColab[] }>(this.apiUrl).pipe(
            tap(respuesta => {
    ...
    private restaurarExpandidos(secciones: SeccionMenuColab[]): SeccionMenuColab[] {
