...
export class FiltroColaboradoresDto {
    @ApiPropertyOptional({ description: 'Buscar por nombre, correo o código' })
    @IsOptional()
    @IsString()
    busqueda?: string;

    @ApiPropertyOptional({ description: 'Filtrar por ID de empresa' })
    @IsOptional()
    @IsNumber()
    @Type(() => Number)
    empresaId?: number;
    ...
    @ApiPropertyOptional({ description: 'Filtrar por ID de almacén asignado' })
    @IsOptional()
    @IsNumber()
    @Type(() => Number)
    almacenId?: number;
